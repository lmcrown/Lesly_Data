function [ OUT ] = Q2_phase_precession_LC(ses)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
if nargin == 0
    ...
    save('Session_Info.mat','sess')
end
sess=Change_ses_2_LC_dirs(ses);
GP = LD_Globals_LC;
...
%%
try
    TR = load(fullfile(GP.Processed_data_dir,[sess.animal '_' sess.name '_TrialInfo.mat']));
catch
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%RIGHT NOW CONTAINS NO INFO ON THETA DELTA
% theta_fname = ses.other.ThetaEEG_FileName;
...
theta_fname=sess.other.ThetaEEG_FileName;
...
ClustSum = load(fullfile(GP.Processed_data_dir,[sess.animal '_' sess.name '_ClustSummarys.mat']));
...
%             plot( new_pos_pha{iCell}(:,2), new_pos_pha{iCell}(:,3)+ 2*pi,'.k')
            ...
            xlabel('pos')
            ylabel('phase')
             title(sprintf('Direc %d %i',iDirect,iCell))
            subplot(4,1,2:3)
%             histogram(new_pos_pha{iCell}(:,2),xpos);
%             set(gca,'XLim',xpos([1 end]))
%             xlabel('pos')
polarhistogram(new_pos_pha{iCell}(:,3),40);
            
%             subplot(5,1,4)
            bins=-pi:0.25:pi;
%             histogram([new_pos_pha{iCell}(:,3)],bins);
%             histogram([new_pos_pha{iCell}(:,3); new_pos_pha{iCell}(:,3) + 2*pi],40);
%             xlabel('phase')
%             h=histogram([new_pos_pha{iCell}(:,3)],bins);
           [f,xi]= ksdensity([new_pos_pha{iCell}(:,3)],bins);
            
%              x=h.BinEdges(1:end-1)/(2*pi);
            x=xi/(2*pi);
            y=f;
...
%             fit = @(b,x)  b(1).*(sin(2*pi*x./b(2) + 2*pi/b(3))) + b(4);    % Function to fit
            fit = @(b,x)(b(1).*(sin(2*pi*x + 2*pi/b(2))) + b(3));
...
%             grid
        axis tight
        Params{iCell}.s=s;
        %s(1) is amplitude, s(2) is period-radians s(3) phase is
        %s(2)/(2*s(3))
        %s(4) is offset
        %I want a goodness of fit measure and to save the peak phase
%         fittedsine=(s(1).*(sin(2*pi*x./s(2) + 2*pi/s(3))) + s(4));
        fittedsine=(s(1).*(sin(2*pi*x + 2*pi/s(2))) + s(3));
        [rho, pval]=circ_corrcc(fittedsine,x);
        Params{iCell}.r=rho;
        subplot(4,1,4)
         plot((x*2*pi),y,'b', x*2*pi,fittedsine, 'r')
         axis tight
        title(sprintf('fit %2.2f',rho))
        end
    end
%% Criteria for exclusion

%based on Yamaguchi 
%1) sufficient nunmber of spikes (200)
%2)approx unimodal distribution of spikes in POSITION
%3)place field not too close to feeding position (locomotion but also no
%spikes when rat is turning around and stuff

[dip,xl,xu, ifault, gcm, lcm, mn, mj] = HartigansDipTest(xpdf)

[dip,xl,xu, ifault, gcm, lcm, mn, mj] = HartigansDipTest(new_pos_pha{iCell}(:,3));

%this will make a bimodalish distribution thing that gives you a dip of 0.8
t = 2*rand(1,2000)-1;
x = (sign(t).*sqrt(abs(t))+1)/2;
hist(x,20)


[dip,xl,xu, ifault, gcm, lcm, mn, mj] = HartigansDipTest(x);

%iCell= 46 some bimodality and you get a dip value of 0.04
%but then running this in the sig thing gives you a non signficant pvalue
%%
